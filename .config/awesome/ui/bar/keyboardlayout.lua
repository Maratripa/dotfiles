...
local function update_status(self)
    ...
    if #self._layout > 0 then
        ...
        -- is lower by one than the froup numbers reported by xkb_get_group_names
    ...
    self.widget:set_markup_silently('<span font="Iosevka Bold 12">' .. text .. '</span>')
end
...
-- venor, file, section and group_idx, which all correspond to the
...
function keyboardlayout.get_groups_from_group_names(group_names)
    ...
    -- For each token in 'tokens', chech if it matches one of the patterns in
...
function keyboardlayout.new()
    ...
    capi.awesome.connect_signal('skb::group_changed',
