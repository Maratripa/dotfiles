...
local function worker(user_args)
    ...
    local font = args.font or 'Iosevka SS14 Bold 12'
    ...
    local function show_battery_status(batteryType)
        awful.spawn.easy_async_with_shell("acpi | grep 'Battery 0'",
        function(stdout, _, _, _)
            naughty.destroy(notification)
            local status = stdout:match("%d%d:%d%d:%d%d .*")
            if not status then status = "Not charging" end
    ...
    function(widget, stdout)
        ...
        if (charge >= 1 and charge < 10) then
            batteryType = "battery-0%s-symbolic"
        ...
        elseif (charge >= 10 and charge < 20) then batteryType = "battery-1%s-symbolic"
        elseif (charge >= 20 and charge < 35) then batteryType = "battery-2%s-symbolic"
        elseif (charge >= 35 and charge < 50) then batteryType = "battery-3%s-symbolic"
        elseif (charge >= 50 and charge <= 65) then batteryType = "battery-4%s-symbolic"
        elseif (charge >= 65 and charge <= 80) then batteryType = "battery-5%s-symbolic"
        elseif (charge >= 80 and charge <= 90) then batteryType = "battery-6%s-symbolic"
        elseif (charge >= 90 and charge <= 100) then batteryType = "battery-full%s-symbolic"
        end

        if status == 'Charging' then
            batteryType = "battery-charging-symbolic"
        else
