    ...
    watch("acpi -i", timeout,
        function(widget, stdout)
            ...
            if (charge >= 1 and charge < 10) then
                batteryType = "battery-warning-fill"
            ...
            elseif (charge >= 10 and charge < 20) then
                batteryType = "battery-empty-fill"
            elseif (charge >= 20 and charge < 50) then
                batteryType = "battery-low-fill"
            elseif (charge >= 50 and charge < 70) then
                batteryType = "battery-medium-fill"
            elseif (charge >= 70 and charge < 90) then
                batteryType = "battery-high-fill"
            elseif (charge >= 90 and charge <= 100) then
                batteryType = "battery-full-fill"
            end

            if status == 'Charging' then
                batteryType = "battery-charging-fill"
            end

            local icon = gears.color.recolor_image(path_to_icons .. batteryType .. ".svg", "#FFFFFF")
